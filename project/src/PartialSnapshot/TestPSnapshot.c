...
int main(){
    ...
    // Test 2:
    printf("Test 2: Snapshot made for 2,4,8 threads, initialized with value 1 in the registers.\n \
...
#pragma omp barrier
    ...
    // Test 3:
    printf("Test 3: Snapshot made for 2,4,8 threads. Thread 0 scans, sleeps for 10 milliseconds, scans again;\n \
    ...
    for (int i = 2; i <= 8; i = i * 2) {
        ...
        if(createPSnapshot(&test3Snapshot, 100, 100,0) == EXIT_FAILURE){
            return EXIT_FAILURE;
        }
        int snap1[i];
        int snap2[i];
...
#pragma omp barrier
    ...
    // Test 4:
    printf("Test 4: Snapshot made for 4,8,12 threads. \n \
...
#pragma omp barrier
    ...
    // Test 5:
printf("Test 5: Snapshot made for 4,8,12 threads with randomly sized and valued registers.\n \
Thread 0 scans, thread 1 scans, sleeps for 10 milliseconds, scans again, finally thread 0 scans again.\n \
...
#pragma omp barrier
#pragma omp parallel default(none) shared(test5Snapshot,snap1,snap2,snap3,snap4,i,thread0Done,thread1Done) num_threads(i)
    {
        int me = omp_get_thread_num();
